...
from classifier.models import shallow2d, load_resnet18, get_weights
from classifier.training import train_model, test_model
...
class Config:
    ...
    network_type: str = 'resnet18'           # network type (shallownet, resnet18)
...
def run(dataset: EEGDataset):
    ...
    if CONFIG.checkpoint_path is not None:
        ...
        trainloader, validloader, testloader = build_dataloader(dataset, batch_size=CONFIG.batch_size, train_rate=CONFIG.train_rate, 
    ...
    if CONFIG.network_type == 'shallownet':
        logger.info("Loading shallowNet...")
        input_size = dataset.spectrograms[0].shape
        if len(dataset.spectrograms[0].shape) > 2:
            input_size = dataset.spectrograms[0][0].shape

        model = shallow2d(nclasses=CONFIG.nclasses, input_size=input_size, 
                          input_channels=CONFIG.input_channels, device=CONFIG.device)
    else:
        model = load_resnet18(nclasses=CONFIG.nclasses, pretrained=CONFIG.pretrained, 
                              device=CONFIG.device, input_channels=CONFIG.input_channels)
    ...
    model = train_model(model, loss_fn, dataloaders, num_epochs=CONFIG.epochs, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ns', '--number_of_subjects', type=int, default=5, help='number of subjects for which the correlation is computed')
