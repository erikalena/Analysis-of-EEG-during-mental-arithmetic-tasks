    ...
    for subj in range(number_of_subjects):
        ...
        for file in files:
            ...
            if (type == 'ms' or type == 'both') and label == 0:
                # discard first 2 seconds of rest
                sample = sample[120*int(fs):]
            ...
            for j in range(n_segments):
                ...
                for i in range(len(data.ch_names)-1):
                    ...
                    if save_spec:
                        ...
                        mwt = compute_wavelet_transform(eeg_data, fs=31, n_cycles=ncycles, freqs=freqs)
                        """
...
def build_dataloader(dataset, batch_size, train_rate=0.8, valid_rate=0.1, shuffle=True, resample=False):
    ...
    for idx, _ in enumerate(dataset):
        ...
        # normalize with respect to each electrode
        for ch in range(dataset_tmp.spectrograms[idx].shape[0]):
            dataset_tmp.spectrograms[idx][ch] = (dataset_tmp.spectrograms[idx][ch] - torch.min(dataset_tmp.spectrograms[idx][ch]))/(torch.max(dataset_tmp.spectrograms[idx][ch]) - torch.min(dataset_tmp.spectrograms[idx][ch]))
